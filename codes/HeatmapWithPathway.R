###
#   File name : HeatmapWithPathway.R
#   Author    : Hyunjin Kim
#   Date      : Aug 20, 2018
...
#
#   Instruction
#               1. Source("HeatmapWithPathway.R")
#               2. Run the function "heatmapWithPathway" - specify the input file (DE result) and output directory
#               3. The heatmap will be generated under the output directory
#
#   Example
#               > source("The_directory_of_HeatmapWithPathway.R/HeatmapWithPathway.R")
#               > heatmapWithPathway(rCntPath="./data/raw_counts.rda",
#                                    dePath="./results/DEA/DESeq2_D7_vs_Control.xlsx",
#                                    fdrThreshold=1e-20,
#                                    lfcThreshold=0.6,
#                                    regulated=c("all", "up", "down"),
#                                    pDisplayNum=Inf,
#                                    outputDir="./results/Heatmap/")
###

heatmapWithPathway <- function(rCntPath="//isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/Christiano/Joanna/data/raw_counts.rda",
                               dePath="//isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/Christiano/Joanna/results/DEA/DESeq2_D7_vs_Control.xlsx",
                               fdrThreshold=1e-70,
                               lfcThreshold=0.6,
                               regulated=c("all", "up", "down"),
                               pDisplayNum=Inf,
                               outputDir="//isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/Christiano/Joanna/results/Heatmap/") {
  ...
  ### assign group info
  groupInfo <- c(rep("Control", 3), rep("D7", 3))
  
  
  ### load the DE genes
  degs <- read.xlsx2(file = dePath, sheetIndex = 1)
  rownames(degs) <- degs[,1]
  degs <- degs[,-1]
  
  
  ### characterize or numerize the factor columns
  degs[1] <- lapply(degs[1], function(x) as.character(x))
  degs[2:ncol(degs)] <- lapply(degs[2:ncol(degs)], function(x) as.numeric(as.character(x)))
  ...
  if(regulated[1] == "all") {
    ...
    degs <- degs[which(degs$log2FoldChange > 0),]
  ...
  degs <- degs[intersect(which(abs(degs$log2FoldChange) > lfcThreshold), which(degs$padj < fdrThreshold)),]
  ...
  ### pathway analysis
  pKegg <- pathwayAnalysis_CP(geneList = rownames(degs), org = "human", database = "KEGG",
                              title = fileName,
                              imgPrint = FALSE, dir = outputDir)
  pGo <- pathwayAnalysis_CP(geneList = rownames(degs), org = "human", database = "GO",
                            title = fileName,
                            imgPrint = FALSE, dir = outputDir)
  ...
  if(nrow(pKegg) > 0) {
    ...
    heatmap_mat_Kegg <- matrix(NA, nrow(pKegg), nrow(degs))
    colnames(heatmap_mat_Kegg) <- degs$Gene_Symbol
    rownames(heatmap_mat_Kegg) <- pKegg$Description
    
    ### fill the matrix with values
    for(i in 1:nrow(degs)) {
      for(j in 1:nrow(pKegg)) {
        if(grepl(rownames(degs)[i], pKegg$geneID[j], fixed = TRUE)) {
          heatmap_mat_Kegg[j,i] <- degs$log2FoldChange[i]
        }
      }
    }
  ...
  if(nrow(pGo) > 0) {
    ...
    ### create an empty heatmap matrix
    heatmap_mat_Go <- matrix(NA, nrow(pGo), nrow(degs))
    colnames(heatmap_mat_Go) <- degs$Gene_Symbol
    rownames(heatmap_mat_Go) <- pGo$Description
    
    ### fill the matrix with values
    for(i in 1:nrow(degs)) {
      for(j in 1:nrow(pGo)) {
        if(grepl(degs$Gene_Symbol[i], pGo$geneID[j], fixed = TRUE)) {
          heatmap_mat_Go[j,i] <- degs$log2FoldChange[i]
        }
      }
    }
  ...
  ### get some lists for heatmap
  sList <- data.frame(egList=rownames(degs),
                      symbolList=degs$Gene_Symbol,
                      fcList=degs$log2FoldChange)
  idx <- sapply(sList, is.factor)
  sList[idx] <- lapply(sList[idx], function(x) as.character(x))
  
  
  ### order genes based on log2FC
  sList <- sList[order(sList$fcList),]
  
  
  ### scaling
  heatmap_mat <- scale_h(rawCnt[sList$egList,], type = "row")
  rownames(heatmap_mat) <- sList$symbolList
  colnames(heatmap_mat) <- as.character(seq(6))
  
  
  ### set colside colors
  uniqueV <- unique(groupInfo)
  colors <- c("gray", "black")
  names(colors) <- uniqueV
  
  
  ### set rowside colors
  fc_col <- sapply(sList$fcList, function(x) {
    if(x >= 0) {
      return("positive")
    } else {
      return("negative")
  ...
  colors2 <- c("skyblue1", "pink")
  names(colors2) <- c("negative", "positive")
  
  
  ### heatmap
  pdf(paste0(outputDir, fileName, "_heatmap.pdf"), width = 15, height = 15)
  par(oma=c(0,0,2,6))
  heatmap.3(as.matrix(heatmap_mat), main = paste0(fileName, "_Heatmap"),
            xlab = "", ylab = "", col=greenred(100),
            scale="none", key=T, keysize=0.8, dendrogram = 'none', trace = 'none',
            labRow = rownames(heatmap_mat), labCol = colnames(heatmap_mat),
            Rowv = FALSE, Colv = FALSE,
            ColSideColors = cbind(colors[groupInfo]),
            RowSideColors = t(cbind(colors2[fc_col])),
            cexRow = 1, cexCol = 1.5, na.rm = TRUE)
  legend("topright", inset = 0.03, xpd = TRUE, title = "Sample Annotation", legend = names(colors), fill = colors, cex = 1.2, box.lty = 0)
  legend("left", inset = -0.01, title = "log2FC", legend = names(colors2), fill = colors2, cex = 1.2, box.lty = 0)
  dev.off()
