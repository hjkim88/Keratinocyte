...
#   Purpose   : Perform DE analysis on the dataset
#
#   Instruction
#               1. Source("DEA.R")
#               2. Run the function "performDEA" - specify the input file (raw count) and output directory
#               3. The DEA result will be generated under the output directory
#
#   Example
#               > source("The_directory_of_DEA.R/DEA.R")
#               > performDEA(rCntPath="./data/raw_counts.rda",
#                            outputDir="./results/DEA/",
#                            fdrThreshold=1e-20)
###

performDEA <- function(rCntPath="//isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/Christiano/Joanna/data/raw_counts.rda",
                       outputDir="//isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/Christiano/Joanna/results/DEA/",
                       fdrThreshold=1e-20) {
  ...
  sList <- data.frame(egList=de$Entrez_ID[which(de$padj < fdrThreshold)],
                      symbolList=de$Gene_Symbol[which(de$padj < fdrThreshold)],
                      fcList=de$log2FoldChange[which(de$padj < fdrThreshold)])
  ...
  heatmap.3(as.matrix(heatmap_mat), main = paste0(fileName, "_Heatmap"),
            ...
            cexRow = 0.1, cexCol = 1.3, na.rm = TRUE)
  ...
  ### heatmap genes
  write.table(rownames(heatmap_mat), file = paste0(outputDir, fileName, "_heatmap_genes.txt"),
